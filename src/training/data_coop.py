...
class CsvDataset(Dataset):
    def __init__(self, args, input_filename, transforms,classnames, img_key, sep="\t"):
        logging.debug(f'Loading csv data from {input_filename}.')
        df = pd.read_csv(input_filename, sep=sep)

        ## me
        self.gt_event_labels = df[args.gt_label_name].tolist()
        self.classnames = classnames
        self.kg_init = args.kg_init
        ## me
    ...
    def __getitem__(self, idx):
        ...
        texts = tokenize( [ str( event_label ) ] )[0]
        gt_event_class_number =  self.classnames.loc[self.classnames == event_label].index[0]

        return images, texts, gt_event_class_number
...
def get_csv_dataset(args, preprocess_fn, classnames, is_train, epoch=0):
...
def get_data(args, preprocess_fns, epoch=0):
    preprocess_train, preprocess_val, classnames = preprocess_fns
    data = {}

    if args.train_data or args.dataset_type == "synthetic":
        data["train"] = get_dataset_fn(args.train_data, args.dataset_type)(
            args, preprocess_train, classnames, is_train=True, epoch=epoch)

    if args.val_data:
        data["val"] = get_dataset_fn(args.val_data, args.dataset_type)(
            args, preprocess_val, classnames, is_train=False)
